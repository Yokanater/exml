import pybullet as p
import os
import math


class Track:
    def __init__(self, client, lap_start, checkpoints):
        self.client = client
        self.lap_start = lap_start
        self.checkpoints = list(checkpoints)
        self.next_checkpoint = 0
        board = [
            "...........................................................................######################...................",
            "................................................................############.....2..............###.................",
            ".........................................................########................2....####........###...............",
            "......................................................####...............##############..#######....##..............",
            "....................................................###.......############...........#######...###...##.............",
            "..................................................###....######....................#.......#.....##...##............",
            "................................................###....###........................#.........#.....##...##...........",
            "..............................................###....###........................#......##...#.......##..##..........",
            ".............................................##....###......######.............#.....##..#..#........##..##.........",
            "...........................................##.....##......###....##...........#.....#...#...#.........##..##........",
            ".........................................##.....##......###.......##.........#.....#....#..#...........##..##.......",
            "........................................##....##......###.....##...#........#.....#....#...#............##..##......",
            "......................................##.....##.....###.....##..#..#........#....#....#....#.............##..#......",
            ".....................................##....##.....###.....##....#..#.......#....#.....#...#...............#..##.....",
            "...................................##.....##.....##.....##......#..#.......#...#.....#...#................##..##....",
            "..................................##....##......##....##.......#...#......#...#......#...#.................##..#....",
            "................................##....##.......##...##.........#...#......#..#......#...#...................#..##...",
            "...............................##....##.......##...#...........#..#......#...#......#...#...................##..##..",
            ".............................##1111##.........#...#............#..#......#...#.....#...#.....................##..#..",
            "...........................##.....##.........#...#.............#..#.....#...#......#...#......................#..##.",
            "..........................##....##...........#..#..............#...#...#....#......#..#.......................##..#.",
            "........................##.....##............#..#..............#....###....#.......#..#........................#..##",
            ".......................##....##..............#..#...............#....#....#........#.#.........................##..#",
            ".....................###....#................#66#...............##...5...##........#4#..........................#33#",
            "...................###....##.................#..#.................#######..........#..#.........................#..#",
            "..................##.p..##...................#..#..................................#..#.........................#..#",
            "................##.....##....................#..#..................................#..#.........................#..#",
            "...............##....##......................#...#.................................#...#........................#..#",
            ".............##....##.........................#..#.................................#...#........................#..#",
            "............##....##..........................#...#.................................#...#.......................#..#",
            "..........##....##............................#....#................................#....#......................#..#",
            "........###....##..............................#.....#...............................#....##....................#..#",
            ".......##....##.................................#......#..............................#....####.................#..#",
            "......##...###...................................#......####...........................#......#####.............#..#",
            "....###...##......................................##........##..........................###.......#####.........#..#",
            "...##....##.........................................###.......####.........................###........#####.....#..#",
            "..##...##..............................................###........###.........................####........######...#",
            ".##...##..................................................###........###..........................####............#.",
            ".#...##......................................................###........###...........................####........#.",
            "#...#...........................................................###........###............................########..",
            "#...#..............................................................####.......####..................................",
            "#..#...................................................................###........###...............................",
            "#..#......................................................................###........##.............................",
            "#...##.......................................................................###.......##...........................",
            "#99999##........................................................................###......##.........................",
            ".#......##.........................................................................##......#........................",
            "..#.......#...........................................................................##....#.......................",
            "....#......#...........................................................................##....#......................",
            "......#....#.............................................................................#....#.....................",
            ".......#...#..............................................................................#...#.....................",
            "........#..#...............................................................................#...#....................",
            ".......#...#...............................................................................#...#....................",
            ".......#..#.................................................................................#..#....................",
            ".......#..#.................................................................................#..#....................",
            ".......#..#.................................................................................#...#...................",
            ".......#..#..................................................................................#..#...................",
            ".......#..#..................................................................................#..#...................",
            ".......#..#..................................................................................#77#...................",
            ".......#..#..................................................................................#..#...................",
            ".......#..#..................................................................................#..#...................",
            ".......#..#..................................................................................#...#..................",
            ".......#...#..................................................................................#..#..................",
            "........#..#..................................................................................#..#..................",
            "........#...#.................................................................................#..#..................",
            "........#...##................................................................................#..#..................",
            ".........#...#...............................................................................##..#..................",
            "..........#...#............................................................................##....#..................",
            "..........#....##...................................................................#######.....#...................",
            "...........#....##..............................................#####################..........##...................",
            "............#.....##.........................####################...........................####....................",
            ".............#......####...###################...............................###############........................",
            "...............#........#####................................##################......................................",
            "................####......8...............###################.......................................................",
            "...................########################.........................................................................",
        ]
        width = max(len(r) for r in board)
        height = len(board)
        cx = width / 2.0
        cy = height / 2.0
        self.spawn_world = None
        for r, row in enumerate(board):
            for c, ch in enumerate(row):
                if ch == 'p':
                    x = c - cx + 0.5
                    y = cy - r - 0.5
                    self.spawn_world = (x, y)
                    break
            if self.spawn_world is not None:
                break
        track_path = os.path.join(os.path.dirname(__file__), 'track.urdf')
        with open(track_path, 'w') as f:
            f.write('<?xml version="1.0" ?>\n')
            f.write('<robot name="track">\n')
            f.write('  <material name="track_mat">\n')
            f.write('    <color rgba="0.6 0.6 0.6 1"/>\n')
            f.write('  </material>\n')
            f.write('  <link name="track_link">\n')
            f.write('    <inertial>\n')
            f.write('      <mass value="0.0"/>\n')
            f.write('      <inertia ixx="0.0" ixy="0.0" ixz="0.0" iyy="0.0" iyz="0.0" izz="0.0"/>\n')
            f.write('    </inertial>\n')
            for r, row in enumerate(board):
                for c, ch in enumerate(row):
                    if ch == '#':
                        x = c - cx + 0.5
                        y = cy - r - 0.5
                        f.write('    <visual>\n')
                        f.write('      <origin xyz="%f %f 0.5" rpy="0 0 0"/>\n' % (x, y))
                        f.write('      <geometry>\n')
                        f.write('        <box size="1 1 1"/>\n')
                        f.write('      </geometry>\n')
                        f.write('      <material name="track_mat"/>\n')
                        f.write('    </visual>\n')
                        f.write('    <collision>\n')
                        f.write('      <origin xyz="%f %f 0.5" rpy="0 0 0"/>\n' % (x, y))
                        f.write('      <geometry>\n')
                        f.write('        <box size="1 1 1"/>\n')
                        f.write('      </geometry>\n')
                        f.write('    </collision>\n')
            f.write('  </link>\n')
            f.write('</robot>\n')
        p.loadURDF(fileName=track_path, basePosition=[0,0,0], physicsClientId=client)

        digit_positions = {}
        for r, row in enumerate(board):
            for c, ch in enumerate(row):
                if ch.isdigit() and ch != '0':
                    x = c - cx + 0.5
                    y = cy - r - 0.5
                    digit_positions.setdefault(ch, []).append((x, y))

        if digit_positions:
            numbered = {}
            for ch, pts in digit_positions.items():
                xs = [p[0] for p in pts]
                ys = [p[1] for p in pts]
                numbered[int(ch)] = (sum(xs) / len(xs), sum(ys) / len(ys))
            self.numbered_checkpoints = numbered
            self.ordered_checkpoint_ids = sorted(self.numbered_checkpoints.keys())
            self._seq_next_index = 0
            self._seq_completed = False
        else:
            self.numbered_checkpoints = None
            self.ordered_checkpoint_ids = None

    def get_spawn(self):
        return self.spawn_world

    def reset(self):
        self.next_checkpoint = 0
        if getattr(self, 'ordered_checkpoint_ids', None):
            self._seq_next_index = 0
            self._seq_completed = False

    def check_and_advance(self, pos):
        if getattr(self, 'numbered_checkpoints', None):
            ids = self.ordered_checkpoint_ids
            if not ids:
                return None
            expected_id = ids[self._seq_next_index]
            cp = self.numbered_checkpoints.get(expected_id)
            if cp is None:
                return None
            d = math.hypot(pos[0] - cp[0], pos[1] - cp[1])
            if d < 1.0:
                if self._seq_next_index == 0 and self._seq_completed:
                    self._seq_completed = False
                    self._seq_next_index = 1 if len(ids) > 1 else 0
                    return 'lap'
                self._seq_next_index += 1
                if self._seq_next_index >= len(ids):
                    self._seq_completed = True
                    self._seq_next_index = 0
                return 'checkpoint'
            return None
        else:
            if self.next_checkpoint < len(self.checkpoints):
                cp = self.checkpoints[self.next_checkpoint]
                d = math.hypot(pos[0] - cp[0], pos[1] - cp[1])
                if d < 1.0:
                    self.next_checkpoint += 1
                    return 'checkpoint'
            else:
                d = math.hypot(pos[0] - self.lap_start[0], pos[1] - self.lap_start[1])
                if d < 1.0:
                    if self.next_checkpoint >= len(self.checkpoints):
                        self.next_checkpoint = 0
                        return 'lap'
            return None
